...
public class GMCountriesPicking : MonoBehaviour
{
    ...
    public void Start()
    {
        if (Menu.gameMode.Contains("Learn"))
        {
            LearnMode(NACountries, NATerritories, "NACountries", -21, -1, -12, -30, 21, -8);
            LearnMode(SACountries, SATerritories, "SACountries", -11, -12, -5, -16, -5.5f, -22.5f);
            LearnMode(AFCountries, AFTerritories, "AFCountries", 4, -7, 13, -4, 0, -17);
            LearnMode(ASIACountries, ASIATerritories, "ASIACountries", 16, -2, 40, 2, 23, -17);
            LearnMode(EUCountries, EUTerritories, "EUCountries", 3, 2, 6, -5, 20, -6f);
            LearnMode(OCCountries, OCTerritories, "OCCountries", 32, -12, 55, 23, -2.5f, -24);
        }
        else
        {
    ...
    void LearnMode(GameObject cHolder, GameObject tHolder, string mode, int x, int y, float maxX, float minX, float maxY, float minY)
    {
        if (gameMode == mode + "Learn")
        {
            ...
            gameEnded = true;
            tempCanvas.SetActive(false);
            gameEndedMenuclone.GetComponent<Resume>().spectatorModeEnabled = true;
            gameEndedMenuclone.GetComponent<Resume>().HideThisObject("Countries");
            goBackButton.GetComponent<Button>().onClick.AddListener(delegate
            {
